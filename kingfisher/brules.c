...
void initBStructures(bitvector ***brules, int Q, int *lastQ, int **cons, int **xfreq, int **xafreq, float **bestMval,
                     int **bestsigns) {
    int i;
...
void addrule(float val, Node *p, Node *par, int cons, int sign, int *kandi, int l) {
    ...
    if (insertBestfromNode(val, cons, par->data->fr, fr, kandi, l, k, BRULES, Q, &LASTQ, CONS, XFR, XAFR, MVAL, BSIGNS,
                           sign) == 1) {
        ADDEDRULES++;
    }
...
int insertBestfromNode(float val, int cons, int xfreq, int xafreq, int *kandi, int l, int k, bitvector **brules, int Q,
                       int *lastQ, int *bcons, int *bxfreq, int *bxafreq, float *bestMval, int *bestsigns, int sign) {
    float cf;
    ...
    /* else add rule*/
    insertrule2(val, cons, xfreq, xafreq, kandi, k, l, brules, Q, lastQ, bcons, bxfreq, bxafreq, bestMval, bestsigns,
                sign);
...
/* for both pos. and neg. rules; signs are in table bestsign */
int
insertrule2(float val, int cons, int xfreq, int xafreq, int *kandi, int k, int l, bitvector **brules, int Q, int *lastQ,
            int *bcons, int *bxfreq, int *bxafreq, float *bestMval, int *bestsigns, int sign) {
    int i;
    bitvector *uusi;
...
int support(int *attr, int **suppArray, int nofrows, int nofcols, int nofattr) {
    int support = 0;
    for (int j = 0; j < nofrows; j++) {
        for (int k = 0; k < nofcols; k++) {
            int inTr = 0;
            for (int i = 0; i < nofattr; i++) {
                if (attr[i] == suppArray[j][k])
                    inTr++;
            }
            if (inTr == nofattr)
                support++;
...
int isPSS(int *attr, int **suppArray, int nofrows, int nofcols, int nofattr, int class) {
    int arr[] = {class};
    int classSupport = support(arr, suppArray, nofrows, nofcols, 1);
    int XSupport = support(attr, suppArray, nofrows, nofcols, nofattr);
    int notClassSupport = nofrows - classSupport;


    if (XSupport <= classSupport || XSupport <= notClassSupport)
        return 1;
...
void
printresults(struct _iobuf *f, int Q, int lastQ, int pstyle, bitvector **brules, int *bcons, int *bxafreq, int *bxfreq,
             float *bestMval, int *bestsigns, int *fr, int *classes, int classesCount, int nofcols, int **suppArray,
             int DCount) {
    int i, j;
...
//            continue;
        ...
        if (bxafreq[i] / ((float) n) < minimumFr)
            continue;

        bool isInRight = false;
        bool isInLeft = false;
        for (int j = 0; j < classesCount; j++) {
            if (bcons[i] == classes[j]) {
                isInRight = true;
                break;
            }
        }
        ...
        int *attr = (int *) malloc(nofattr * sizeof(int));
        int jj = 0;
        for (j = 0; j < k; j++) {
            if (bv_tstbit(brules[i], j)) {
                if (j != bcons[i]) {
        ...
        if (isPSS(attr, suppArray, DCount, nofcols, nofattr, bcons[i]) == 0)
            continue;
        ...
        for (j = 0; j < k; j++) {
            if (bv_tstbit(brules[i], j)) {
                if (j != bcons[i]) { /* print condition part */
                    for (int jj = 0; jj < classesCount; jj++) {
                        if (j == classes[jj]) {
                            isInLeft = true;
                            break;
                        }
        ...
        if (pstyle == 1)
            fprintf(f, " fr=%d (%.4f), cf=%.3f, gamma=%.3f, delta=%.3f, M=%.3e\n", bxafreq[i], bxafreq[i] / ((float) n),
                    cf[i], gamma[i], delta[i], bestMval[i]);
        else fprintf(f, "\n");
    } /* for i */
...
void calctatistics(int num, int *bfr, float *cf, float *gamma, float *delta, float *bestMval, int *bestsigns, int n) {
    ...
    printf("Among %d best rules %d negative rules, avg(M)=%.2e stdev(M)=%.2e\n  avg(cf)=%.3f stdev(cf)=%.3f avg(gamma)=%.3f stdev(gamma)=%.3f\n  avg(delta)=%.3f stdev(delta)=%.3f avg(fr)=%.1f stdev(fr)=%.1f\n",
           num, nrules, avgM, sqrt(Msum / num), avgcf, sqrt(cfsum / num), avgg, sqrt(gsum / num), avgd,
           sqrt(dsum / num), avgfr, sqrt(frsum / num));
}
