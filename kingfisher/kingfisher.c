...
int certainkandi(int *kandi, int l) {
    if ((l >= 2) && (((kandi[0] == 464) && (kandi[1] == 747)) || ((kandi[0] == 464) && (kandi[1] == 470)) ||
                     ((kandi[0] == 747) && (kandi[1] == 470))))
        return 1;
...
int generate1sets(bitvector **amatr, Node *t, int **fr, int **oldaddr, int **order, int *special) {
    ...
    printf("Data matrix takes %d bits (%.1f MB) space; freed %d bits (%.1f MB)\n", n * (k - nol),
           n * (((float) k) - nol) / (8.0 * 1024.0 * 1024.0), n * nol, n * ((float) nol) / (8.0 * 1024.0 * 1024.0));
...
int
visit(Node *p, Node *t, int len, int l, int *next, int **kandi, int *order, bitvector **amatr, int *fr, int *oldaddr, int k, int *suppArray, int DSize) {
    int i, j;
    Node *child;
    int *par2set;
    ...
    if (len == l - 2) { /* path length l-2, i.e. we are in a leaf (do nothing) or */
        ...
        for (i = 0; i < p->chsize; i++) { /* polku kesken, jos lapsia. */
            if (p->children[i] != NULL) {
                (*kandi)[len] = p->children[i]->label;
                if ((visit(p->children[i], t, len + 1, l, next, kandi, order, amatr, fr, oldaddr, 0, NULL, DSize) == 0) && (len > 0))
                    delChild(p, p->children[i]->label, order);
            }
        } /* for */
...
int intervisit(Node *p, Node *t, int len, int l, int *next, int **kandi, int *order, bitvector **amatr, int *fr,
               int *oldaddr, int iinpath) {
    int i, j;
    Node *child;
    int *par2set;
    int oldiinpath;
    ...
    if (len == l - 2) { /* path length l-2, i.e. we are in a leaf (do nothing) or */
        ...
        if ((p->chsize >= 2) && ((iinpath) || (interchildren(p))))
            for (i = 0; i < (p->chsize) - 1; i++)
                if (p->children[i] != NULL) {
                    ...
                    for (j = i + 1; j < (p->chsize); j++)
                        if ((p->children[j] != NULL) &&
                            ((eok == 0) || (constraints[p->children[i]->label][p->children[j]->label] == 0))) {
        ...
        for (i = 0; i < p->chsize; i++) { /* polku kesken, jos lapsia. */
            if (p->children[i] != NULL) {
                ...
                if (interattr[p->children[i]->label]) iinpath = 1;
                (*kandi)[len] = p->children[i]->label;
                if ((intervisit(p->children[i], t, len + 1, l, next, kandi, order, amatr, fr, oldaddr, iinpath) == 0) &&
                    (len > 0))
                    delChild(p, p->children[i]->label, order);
                iinpath = oldiinpath;
            }
...
int generatecandidates(Node *t, int l, int *order, bitvector **amatr, int *fr, int *oldaddr, int k, int *suppArray, int nofrows) {
    int i, iinpath = 0;
    int *kandi;
    int next = 0;
    float prevvalM = valM;
    ...
    if (l == 2) { /* level 2 */
        if (intergiven == 0)
            visit(t, t, 0, l, &next, &kandi, order, amatr, fr, oldaddr, k, suppArray, n);
        else
      ...
      prevlnp=maxlnp;
        ...
        for (i = 0; i < t->chsize; i++) /* for all branches */
            if (t->children[i] != NULL) {
                ...
                if (intergiven == 0)
                    visit(t->children[i], t, 1, l, &next, &kandi, order, amatr, fr, oldaddr, 0, NULL, n);
                else {
...
int prunecandpF(Node *t, Node *p, bitvector **amatr, int l, int *fr, int *kandi, int *order, int *oldaddr) {
        ...
        for (i = 0; i < p->data->parsize; i++)
            if (p->data->parlabels[i] >= 0) {
                ...
                if (isposav(p, oind)) {
                    ...
                    if ((p->data->fr < posabsminfrx[oind]) ||
                        ((lnlb = lnmoverl(n - p->data->fr, n - fra)) >= p->data->pbest[indinkandi] + EPSILON) ||
                        (lnlb > maxlnrx[oind]) ||
                        ((best = lnmoverl(n - p->data->fr, n - fra)) >= p->data->pbest[indinkandi] + EPSILON) ||
                        (best > maxlnrx[oind])) {
                        ...
                        if (p->data->fr == fry) { /* cf=1 */
                            ...
                            checkrule(p, p->data->parents[i], fry, p->data->fr, fra, p->data->parlabels[i], 1, order,
                                      kandi, l);
                        }
        ...
        for (i = p->data->parsize - 1; i >= 0; i--)
            if (p->data->parlabels[i] >= 0) {
                ...
                if (isnegav(p, oind)) {
                    ...
                    if ((fry - p->data->fr < negabsminfrx[oind]) ||
                        ((lnlb = lnmoverl(n - fry + p->data->fr, fra)) >= p->data->nbest[indinkandi] + EPSILON) ||
                        (lnlb > maxlnrx[oind]) ||
                        ((best = lnmoverl(n - fry + p->data->fr, fra)) >= p->data->nbest[indinkandi] + EPSILON) ||
                        (best > maxlnrx[oind])) {
                        ...
                        checkrule(p, p->data->parents[i], fry, fry - p->data->fr, n - fra, p->data->parlabels[i], 0,
                                  order, kandi, l);
                    }
                }
    ...
    if (cf1found == 1) { /* set others to 0 */
        ...
        if (NEG) {
            for (i = 0; i < p->data->nposlen; i++) /* for all negative consequents */
                if ((bv_tstbit(p->data->npossible, i) == 1) &&
                    (occursinset(labelgivennegind(i, oldaddr), kandi, l) == 0)) {
                    /* if A is not in set X */
    ...
    if (((NEG == 0) && (p->data->pposlen == 0)) || ((POS == 0) && (p->data->nposlen == 0)) ||
        ((p->data->pposlen == 0) && (p->data->nposlen == 0))) {
        return 0;
    }
    return 1;
}
        ...
        for (i = p->data->parsize - 1; i >= 0; i--)
            if (p->data->parlabels[i] >= 0) {
                ...
                if (isnegav(p, oind)) {
                    ...
                    /* allow some error in precision in redundancy checking */
                    if ((fry - p->data->fr < absminfr) || (better(valM, val) == 1) ||
                        (better(p->data->nbest[indinkandi] + EPSILON, val) == 1)) {
                        /* set as NAV to p and par */
                        setnegnav(p, oind);
                        setnegnav(p->data->parents[i], oind);
                    } else {/* check rule */
        ...
        for (i = p->data->nposlen - 1; i >= 0; i--) {
            ...
            if (indinkandi < 0)
                ...
                if (bv_tstbit(p->data->npossible, i)) { /* was AV */
                    ...
                    /* now m(~XA)=m(A) */
                    if ((par->data->fr >= negabsminfrx[index]) &&
                        ((best = lnmoverl(n - par->data->fr, fr[label])) < p->data->nbest[indinkandi] + EPSILON) &&
                        (best <= maxlnrx[index])) {
                        ...
                        addrule(best + abses[index], p, par, label, 0, kandi,
                                l); /* adds to BRULES, if sufficiently good */
                    }
...
void checkLowFrpF(Node *p, int *fr, int *oldaddr, int *order, int *kandi, int l) {
    ...
    for (i = p->data->nposlen - 1; i >= 0; i--) {
        ...
        if (bv_tstbit(p->data->npossible, i)) { /* was AV */
            ...
            if ((par->data->fr - p->data->fr < negabsminfrx[index]) ||
                ((best = lnmoverl(n - par->data->fr + p->data->fr, fr[label])) >=
                 p->data->nbest[indinkandi] + EPSILON) || (best > maxlnrx[index])) {
...
int main(int argc, char **argv) {
    ...
    i = 0;
    POS = 1;
    NEG = 0; /* by default, only positive rules */
    ...
    while ((opt = getopt(argc, argv, "g:i:k:M:c:o:m:l:q:p:w:t:e:b:f:ud")) != -1) {
        /* option a disabled */
        ...
        switch (opt) {
            case 'g':
                ...
                classesCount = 1;
                for (int i = 0; optarg[i] != NULL; i++) {
                    if (optarg[i] == '-')
                        classesCount++;
                }
                classes = (int *) malloc(classesCount * sizeof(int));
            ...
            case 'p':
                ...
                break;
            case 'e':
                printf("Compatibility constraints in file [%s]\n", optarg);
            ...
            case 'b':
                printf("Special constraints in file [%s]\n", optarg);
	...
	error(1);
      ...
      break; */
            ...
            case 'f':
                printf("Fixed consequences in file [%s]\n", optarg);
    ...
    suppArray = (int **)malloc(n * sizeof(int));
    int nofcols = initattrmatr(f, &amatr, n, k, &suppArray);
  ...
  for (i=0;i<numl;i++)
    ...
    while ((numl >= l) && (l <= MAXTASO)) {
        ...
        ADDEDRULES = 0;
        tim1 = clock();
        numl = generatecandidates(t, l, order, amatr, fr, oldaddr, k, suppArray, n);
        tim2 = clock();
        tottim += (double) (tim2 - tim1);
    ...
    if (outok) { /* print to output file */
        printresults(f2, Q, LASTQ, pstyle, BRULES, CONS, XAFR, XFR, MVAL, BSIGNS, fr, classes, classesCount, nofcols,
                     suppArray, n);
        if (fclose(f2) == EOF)
            error(4);
    } else { /* print to stdout */
        printresults(stdout, Q, LASTQ, pstyle, BRULES, CONS, XAFR, XFR, MVAL, BSIGNS, fr, classes, classesCount, nofcols, suppArray, n);
    }
