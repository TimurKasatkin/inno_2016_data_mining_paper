...
void initializeMeasure(int Mnum){
  ...
  if (INC==0){ /* decreasing measures */
    better=better1;
    /* changed 3.5.15 
    EPSILON=-1.0*EPSILON;  set negative */
  }
...
double ub2MI(int frx,int fra,int nn){
  ...
  if (fra<=frx) /* best possible value, non-informative ub */
    return (nn*log2f((double)nn)-fra*log2f((double)fra)-(nn-fra)*log2f((double)nn-fra));
  else /* frx<fra */
    return (nn*log2f((double)nn)+(fra-frx)*log2f((double)fra-frx)-(nn-frx)*log2f((double)nn-frx)-fra*log2f((double)fra));
 }
...
double ub3MI(int frxa,int frx,int fra,int nn){
  ...
  return (nn*log2f((double)nn)+(fra-frxa)*log2f((double)fra-frxa)-fra*log2f((double)fra)-(nn-frxa)*log2f((double)nn-frxa));
