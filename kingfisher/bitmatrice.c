...
/* luodaan bittimatriisi, jossa attribuutit riveill� ja datarivit sarakkeina */
int initattrmatr(FILE *f, bitvector ***matr, int n, int k, int ***supArray) {
    ...
    while (i < n) {
        ...
        for (int ii = 0; row[ii] != NULL; ii++)
            if (row[ii] == ' ')
                nofcols++;
        /* read first attr */
        v = strtol(row, &endp, 10);
        (*supArray)[i] = (int *) malloc(nofcols * sizeof(int));
        (*supArray)[i][0] = v;
        ...
        bv_setbit((*matr)[v], i); /* ol. ett� attr. numerointi alkaa 0:sta */
